...
class AllocationAlgBase:
    ...
    def allocate_good(
        ...
        '''
        Allocate the new good using the greedy algorithm.
        '''
    ...
    def test_allocation(self, test_good) -> float:
        ...
        foo = np.min(self.get_prop1_alpha())
...
class AllocationAlgGreedyTwo(AllocationAlgBase):
    def __init__(
            self,
            rng_seed: int | None = None,
            use_prop_1: bool = False,
            N: int = 50,
            adv_strat: str = 'greedy'
                 ):
        super().__init__(num_agents=2, rng_seed=rng_seed, use_prop_1=use_prop_1, N=N)

        self.adv_strat = adv_strat
    ...
    def find_next_good(
        ...
        '''
        if self.adv_strat == 'mixer':
            if self.t % 10 < 3:
                return [self.rng.uniform(), self.rng.uniform()]
            else:
                return self.find_worst_good()
        elif self.adv_strat == 'greedy':
            return self.find_worst_good()
        elif self.adv_strat == 'random':
            return [self.rng.uniform(), self.rng.uniform()]
    ...
    def update_fig(self, t: int):
        ...
        next_good = self.find_next_good()
        self.allocate_good(next_good)
        self.alpha_tracker.append(np.min(self.get_prop1_alpha()))
...
def main():
    alloc = AllocationAlgGreedyTwo(rng_seed = 123)
